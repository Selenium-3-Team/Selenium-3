...
public interface IWaiter extends IFinder {
	 ...
	 * 
	 * @param status    - element's status, start it with "displayed, not
	 *                  displayed,..."
	 * @param timeOut   - a time out in seconds
	 * @param throwable - throwable status: True or False, if waits of element are
	 *                  false. it will throw a exception
	 */
	public default void waitForCondition(Status status, int timeOut, boolean throwable) {
		try {
			...
			logger.severe(String.format("Has error when wait for element %s '%s': %s", status.toString().toLowerCase(),
					getLocator().toString(), error.getMessage()));
			if (throwable) {
				if (error instanceof TimeoutException) {
					throw new TimeoutException(
							String.format("Timeout when wait for element '%s' is '%s' within %s seconds",
