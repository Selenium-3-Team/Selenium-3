...
public class JsonHelper {
	 ...
	 * 
	 * @example {@code String jsonString = "{'Selenium': '3', 'Team': '4'}";} //Create the json string
	 ...
	 * @throws JsonSyntaxException if json is not a valid representation for an object of type
	 * 
	 * @example {@code File file = new File("\\data.json");} //File path to data.txt/data.json (not required *.json file)
	 ...
	 * 			{@code Type mapType = new TypeToken<List<Map<String, String>>>() {}.getType();} //Create Type type variable, use to convert from json to type
	 * 			{@code List<Map<String, String>> info = JsonHelper.getListData(path, mapType);} //Convert to list map, return List<Map<String, String>>
	 * 			{@code info.get(0).get("Selenium"); info.get(0).get("Team");} //Get data in list map
	 */
	 ...
	 * 			Create a data.json file and enter the values as follows: [{"Selenium": 3, "Team": 4}]
	 * 			{@code String path = file.getAbsolutePath();} //Get absolute path of data file
	 * 			{@code JsonObject info = JsonHelper.getJsonObject(path);} //Convert to json object, return JsonObject
	 * 			{@code info.get("Selenium").toString(); info.get("Team").toString();} //Get data in json object
	 */
	 ...
	 * 
	 * @example {@code String jsonString = "{'Selenium': '3', 'Team': '4'}";} //Create the json string
	 * 			{@code JsonElement jsonElement = JsonHelper.getJsonObjectFromString(jsonString);} //Convert to json element, return JsonElement
	 * 			{@code JsonObject info = jsonElement.getAsJsonObject();} //Convert to json object, return JsonObject
	 * 			{@code info.get("Selenium").toString(); info.get("Team").toString();} //Get data in json object
	 */
	 ...
	 * @throws JsonSyntaxException if json is not a valid representation for an object of type
	 * 
	 * @example {@code String jsonString = "{\"browserName\": \"Chrome\", \"browserVersion\": \"105.0.5195.127\"}";} //Create the json string
	 ...
	 * 			Create a data.json file and enter the values as follows: [{"Selenium": 3, "Team": 4}]
	 * 			{@code String path = file.getAbsolutePath();} //Get absolute path of data file
	 * 			{@code JsonReader reader = getJsonReader(path);} //Convert to json reader, return getJsonReader
	 */
